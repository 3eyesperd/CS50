...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        // prompt for word and capitalize it
        printf("> ");
        string s = GetString();
...
{    
    int lower_limit = 0;
    int upper_limit = WORDS;
    int search =(lower_limit + upper_limit)/2;
    
    
    while (lower_limit != upper_limit)
    {
        if (strcmp(s, dictionary.words[search].letters) > 0)
        {
            lower_limit = search;
            search = (search + upper_limit)/2;
        }
        else if (strcmp(s, dictionary.words[search].letters) < 0)
        {
            upper_limit = search;
            search =(lower_limit + search)/2;
        }
        else
        {
            if ( dictionary.words[search].found == true )
                return false;
            else           
            {
                dictionary.words[search].found = true;
                return true;
            }   
        }                   
    }
...
void scramble(void)
{
    char grid_tmp[DIMENSION][DIMENSION];
    
    for (int row = 0; row < DIMENSION; row ++)
        for (int col = 0; col < DIMENSION; col++)
            grid_tmp[row][DIMENSION - 1 - col] = grid[col][row];
    for (int row = 0; row < DIMENSION; row ++)
        for (int col = 0; col < DIMENSION; col++)
            grid[row][col] = grid_tmp[row][col];
